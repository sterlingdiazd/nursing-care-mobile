...
import { useNurseServiceCalendar, type CalendarView } from "@/src/hooks/useNurseServiceCalendar";
...
export default function NurseServiceCalendarScreen() {
  const { isReady, isAuthenticated, requiresProfileCompletion, roles } = useAuth();
  const isEnabled = isReady && isAuthenticated && !requiresProfileCompletion && roles.includes("NURSE");

  const cal = useNurseServiceCalendar(isEnabled);

  useEffect(() => {
    if (!isReady) return;
    if (!isAuthenticated) router.replace("/login");
    else if (requiresProfileCompletion) router.replace("/register");
    else if (!roles.includes("NURSE")) router.replace("/");
